...
const getAllAnimals = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError("Fetching animals failed", 500);
    return next(error);
  }
  res.json({
...
const getPostedAnimalsByUserId = async (req, res, next) => {
  ...
  res.json({
    animals: userWithAnimals.postedAnimals.map((animal) =>
...
const getFosteredAnimalsByUserId = async (req, res, next) => {
  ...
  res.json({
    animals: userWithAnimals.fosteredAnimals.map((animal) =>
      animal.toObject({ getters: true })
    ),
  });
};
...
const getAdoptedAnimalsByUserId = async (req, res, next) => {
  ...
  res.json({
    animals: userWithAnimals.adoptedAnimals.map((animal) =>
      animal.toObject({ getters: true })
    ),
  });
};
...
const updateAnimalById = async (req, res, next) => {
  ...
  const { name, info, type, imageURL, currentlyStyaingWith, status, userId } =
    req.body;
  const animalId = req.params.aid;
  let animal;
